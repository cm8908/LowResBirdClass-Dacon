...
def train_hr(args, logger, backbone_model, optimizer, criterion, device, train_fold_df, val_fold_df, fold_idx):
    ...
    for e in range(args.num_epochs):
        ...
        backbone_model.eval()
        with torch.no_grad():
            for i, (_, upscale_img, label) in enumerate(val_loader):
                # Move data to device
                upscale_img = upscale_img.to(device)
                label = label.to(device)
        ...
        logger.writer.flush()
        log_str = f'===== Validation E{e},  F1 score (HR): {val_f1score_hr:.4f}  ====='
        logger.logfile.write(log_str + '\n')
        print(log_str)

        if args.early_stop:
            logger.check_early_stop(val_f1score_hr, backbone_model, optimizer, e)
            if logger.stop:
                logger.save_best_checkpoint(f'best_fold{fold_idx}')
                break
    return val_f1score_hr
...
def main(args):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df['label'])):
        ...
        # Begin training loop
        val_f1_hr = train_hr(args, logger, backbone_model, optimizer, criterion, device, train_fold_df, val_fold_df, fold_idx)
